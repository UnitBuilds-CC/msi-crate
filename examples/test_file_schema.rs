//! Test: File table with CORRECT column types per MSI spec
//! 
//! Previous test showed File table triggers error 2725.
//! The Sequence column was Int16 but MSI spec requires Int32.
//! Also, File_ and Component_ should be id_string(72), not string(8).

use std::process::Command;
use velocity_msi::{MsiBuilder, Column, Value};

fn main() {
    println!("=== Test: File table with correct schema ===\n");
    ...
    // File table - CORRECTED SCHEMA
    // Per MSI spec:
    //   File_: identifier, 72 chars max (not 8!)
    //   Component_: identifier, 72 chars max (not 8!)
    //   FileName: filename, 255 chars
    //   FileSize: integer (4 bytes = Int32)
    //   Attributes: integer (2 bytes = Int16), nullable
    //   Sequence: integer (4 bytes = Int32, NOT Int16!)
    builder.create_table("File", vec![
        ...
        Column::build("Attributes").int16().nullable().build(),
        Column::build("Sequence").int32().build(),  // CHANGED: Int16 → Int32
    ]).unwrap();
    ...
    let path = "C:\\temp\\file_schema_test\\correct_file.msi";
    std::fs::create_dir_all("C:\\temp\\file_schema_test").ok();
    ...
    // Now test WITHOUT _Validation to see if it matters
    println!("\n=== Test: File table without _Validation ===");
    
    // We can't easily disable _Validation in MsiBuilder, so let's just note this for now
    println!("(Skipped - would need to modify MsiBuilder)");
}
