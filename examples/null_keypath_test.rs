/// Test: Component with KeyPath=Null (no file reference)
...
fn main() {
    ...
    // Component with KeyPath=Null (like registry/env components)
    ...
    b.insert_rows("Component", vec![
        vec![Value::from("comp_0"), Value::Null, Value::from("INSTALLDIR"), Value::Int(0), Value::Null, Value::Null],
    ]).unwrap();
    ...
    if code == 0 {
        println!("SUCCESS! Null KeyPath works!");
